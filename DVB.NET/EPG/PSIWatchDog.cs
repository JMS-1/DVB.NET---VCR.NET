...
namespace JMS.DVB.EPG
{
	...
	public class PSIWatchDog: IDisposable
	{
		...
		/// </summary>
		private Parser m_Parser = new Parser(null);
		...
		private void AnalysePAT(Section section)
		{
			...
			// Expect a PAT
			Tables.PAT pat = section.Table as Tables.PAT;

			// Verify table
			if ((null == pat) || !pat.IsValid) return;

			// Check all programs
			foreach (KeyValuePair<ushort, ushort> service in pat.ProgramIdentifier)
				lock (m_Stations)					
				{
					// Load the station
					Station station;
