...
(function (JMSLib) {
                ...
                if (attributeTarget == null)
                    if (target.is(':checkbox')) {
                        ...
                        if (element.getAttribute(HTMLTemplate.writebackAttribute) != null)
                            target.change(function () { return model[dataProperty] = target.prop('checked'); });
                    }
                    else
                        target.text(dataValue);
                else
                    $.each(attributeTarget.split(','), function (i, oneAttributeTarget) { return target.attr(oneAttributeTarget, dataValue); });
            });
    ...
    var DateFormatter = (function () {
        ...
        // Ermittelt ein Datum ohne Jahresangabe
        DateFormatter.getShortDateUtc = function (start) {
