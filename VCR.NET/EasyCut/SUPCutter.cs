...
namespace EasyCut
{
    ...
    public class SUPCutter : CutterBase
    {
        ...
        /// <summary>
        /// Kopiert die ISO Datei mit den Farbinformationen.
        /// </summary>
        ...
        private void CopyIfo( string source, string target )
        {
            ...
            try
            {
                using (var ifo = new FileStream( target, FileMode.Open, FileAccess.Read, FileShare.None ))
                {
                    ...
                    // Adjust to color table
                    var colorBias = pgciBase + start + 0xa4;
                    if (colorBias >= ifo.Length)
                        return;
        ...
        private static int ReadNumber( byte[] sector, int offset )
        {
            ...
            var b2 = sector[offset++];
            var b1 = sector[offset++];
            var b0 = sector[offset++];
